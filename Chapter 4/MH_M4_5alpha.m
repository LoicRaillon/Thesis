...
Nall                        = Np + Nx + Ny + Nx;                            % Number of admissible parameters
fix                         = [true(Np + Nx + Ny + (Nx-Ny),1) ; false];     % Logical vector, false = fixed
fix(4)                      = false;                                        % fix Rs
fix(9)                      = false;                                        % fix Cs
fix(18)                     = false;                                        % fix sigw44
Nukwn                       = sum(fix);                                     % number of unknown
sys.fun                     = @M4_5alpha;                                   % Model function
sys.H                       = [0 0 0 1];                                    % Output matrix
sys.dim                     = [Np Nx Ny Nu];                                % Vector of dimension
...
sys.Psqrt                   = diag([0.5 0.5 0.5 0.1]);                      % Initial standard deviation of states
nMC                         = 3;                                            % number of runs
...
prdm                        = zeros(Nukwn,1);                               % Allocation for random parameters init (theta0)
lb                          = [1e-3 1e-4 1e-4 -Inf 1e-4 1e-3 1e-4 1e-2 ...  % lower bounds of [Ro Ri Rm Rs Rz Cw Ci Cm ...
                                -Inf 1e-3 1e-3 1e-2 1e-2 1e-2 -Inf -Inf ... % ... Cs awW awS aiW aiE aiS sigw1 sigw2 ...
                                -Inf -Inf -Inf 20 20 20 -Inf]';             % ... sigw3 sigw4 sigv xw0 xm0 xi0 xs0]
ub                          = [1e-1 5e-2 1e-1 Inf 1e-1 5e-1 1e-1 5 ...      % upper bounds of [Ro Ri Rm Rs Rz Cw Ci Cm ...
                                Inf 1e-1 1e-1 1 1 1 Inf Inf ...             % ... Cs awW awS aiW aiE aiS sigw1 sigw2 ...
                                Inf Inf Inf 40 40 40 Inf]';                 % ... sigw3 sigw4 sigv xw0 xm0 xi0 xs0]
sys.lb                      = lb;                                           % lower bounds
sys.ub                      = ub;                                           % upper bounds
...
idx3                        = false(Nall,1);                                % Logical indexing initial states
idx3(Np+Nx+Ny+1:Nall)       = fix(Np+Nx+Ny+1:Np+Nx+Ny+Nx);                  % ...
eta                         = zeros(Nall,1);                                % Allocation unconstrained paramters
eta(~fix)                   = [2.52e-3 ; 2.52e-4 ; 1e-6 ; y(1)];            % Set fixed parameters
...
for i = 1:nMC   
    ...
    eta(idx1)               = transform(lb(idx1) + (ub(idx1)-lb(idx1)).*betarnd(2,2,[sum(idx1),1]),'LowUp',lb(idx1),ub(idx1));
    % random draw from Gamma(2,0.03) for the std parameters (section 4.2.2.3)
    eta(idx2)               = transform(gamrnd(2,0.03,[sum(idx2),1]),'Log'); 
    % random draw from Beta(2,2,lb,ub) for the RC parameters (section 4.2.2.3)
    eta(idx3)               = transform(lb(idx3) + (ub(idx3)-lb(idx3)).*betarnd(2,2,[sum(idx3),1]),'LowUp',lb(idx3),ub(idx3));     
