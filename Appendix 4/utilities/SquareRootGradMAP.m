% inputs:
%   - eta: unconstrained parameter space
%   - sys: structure with data, constants, options, etc
% outputs
%   - out: structure 
%       - LP: Log posterior distribution(eta)
%       - dLP: gradient(eta)
...
function [LP,dLP] = SquareRootGradMAP(eta,sys)
%% ------------------------------------------------------------------------
% Get information
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
theta                           = eta;
[theta(idx1),J1,dJ1,~]          = inv_transform(eta(idx1),'LowUp',lb(idx1),ub(idx1));
[theta(idx2),J2,dJ2,~]          = inv_transform(eta(idx2),'Log'); 
[theta(idx3),J3,dJ3,~]          = inv_transform(eta(idx3),'LowUp',lb(idx3),ub(idx3));
...
% -------------------------------------------------------------------------
[pB,dpB,~]                      = LogBeta(theta(idx1),2,2,lb(idx1),ub(idx1));    
[pG,dpG,~]                      = LogGamma(theta(idx2),2,0.03); 
[pN,dpN,~]                      = LogBeta(theta(idx3),2,2,lb(idx3),ub(idx3));    
Prior                           = sum(pB) + sum(pG) + sum(pN);              
dPrior                          = [dpB ; dpG ; dpN];
LP                              = LL - Prior;                               % log(posterior) = log(likelihood) + log(prior)
[pZ,~]                          = LogBeta(theta(7),2,2,lb(7),ub(7));        % Prior evaluation of grid parameter (Ci)
% [pZ,~]                          = LogGamma(theta(15),2,0.03);             % Prior evaluation of grid parameter (sigw33)
LP                              = LP - pZ;                                  % Prior of grid parameter
