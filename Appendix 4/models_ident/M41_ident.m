function out = M41_ident(sys)

Np                  = 11;                                                   % Number of R,C parameters
Nx                  = 4;                                                    % Number of states
Ny                  = 1;                                                    % Number of outputs
Nu                  = 4;                                                    % Number of inputs
Ns                  = max(size(sys.y));                                     % Number of samples
idxu                = logical([1 1 1 1]);                                   % indexing inputs
...
sys.fun             = @M41;                                                 % model function, M41
...
xi0                 = [10   ; 30   ; 50];
xs0                 = [10   ; cte  ; 50];
gather              = [Ro Ri Rm Rs Rz Cw Ci Cm Cs aW aI sigw1 sigw2 ...     % Gether in one variable
                        sigw3 sigw4 sigv xw0 xm0 xi0 xs0];                  % ...
Vars                = {'Ro' 'Ri' 'Rm' 'Rs' 'Rz' 'Cw' 'Ci' 'Cm' 'Cs' 'aW'... % parameter names
                       'aI' 'sigw1' 'sigw2' 'sigw3' 'sigw4' 'sigv' 'xw0'... % ...
                       'xm0' 'xi0' 'xs0'}';                                 % ...
...
fun                 = @(x) SquareRootGradPen([x(1:19) ; cte], sys);         % Loglikelihood with penalty function
